...
%  Action: LightOn
act( turnOn(L), % There is only one lightswitch per room, so which is implicit
[shakey(S), switch(L) ,box(B), at(S, L), at(B, L), on(S, box), lights(L, off)], %
[lights(L, off)],
[lights(L, on)]
).

%  Action: LightOff
act( turnOff(L), % There is only one lightswitch per room, so which is implicit
[shakey(S), switch(L), box(B), at(S, L), at(B, L), on(S, box), lights(L, on)], %
[lights(L, on)],
[lights(L, off)]
).
